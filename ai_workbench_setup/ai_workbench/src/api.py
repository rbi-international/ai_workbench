...
def initialize_components():
    ...
    global crowdsource, fusion, collaboration, ethics, explainability
...
app = FastAPI(
    ...
    description="A comprehensive AI platform for text processing",
    version="2.0.0",
    lifespan=lifespan
)
...
# Main processing endpoint - FIXED VERSION
@app.post("/process")
async def process_task(input: TaskInput):
    """Main task processing endpoint - FIXED to preserve text formatting"""
    try:
        ...
        # Retrieve context for chat tasks
        context = []
        ...
        if input.task == "summarization":
            ...
            if evaluator and input.reference:
                try:
                    ...
                    log_info("✓ Summarization evaluation completed")
                except Exception as e:
            ...
            if evaluator and input.reference:
                try:
                    ...
                    if evaluation is not None:
                        evaluation = evaluation.to_dict(orient="records")
                    log_info("✓ Translation evaluation completed")
                except Exception as e:
                    log_warning(f"Evaluation failed: {e}")
            ...
            # DON'T apply clean_response_text here - it's causing the issue
            if results and results[0].get("output"):
                response = results[0]["output"]
                # Only basic cleaning - remove extra whitespace but preserve structure
...
if __name__ == "__main__":
    log_info("🤖 AI Workbench API")
    log_info("🚀 Ready to serve requests!")
